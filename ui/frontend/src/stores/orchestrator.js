  ...
  function init() {
    const attachSocket = (transports) => {
      ...
      socket.value.on("disconnect", () => {
        ...
        setError(
          `Lost connection to the backend. Check that it is running on ${apiBaseUrl}.`,
        );
      ...
      socket.value.on("connect_error", (error) => {
        ...
        setError(
          `Unable to reach the backend (WebSocket/polling). URL: ${socketBaseUrl}${socketPath} via [${transports.join(", ")}]. Ensure the server at ${apiBaseUrl} is accessible.`,
        );
        status.value = "error";
