"""
Graph Store - Core graph database for the context system.

Implements a SQLite-backed graph database with:
...
class GraphStore:
    """SQLite-backed graph database for context storage."""

    def __init__(self, db_path: str | None = None):
        """Initialize the graph store.

        Args:
            db_path: Path to SQLite database file. Defaults to ~/.ai-orchestrator/context.db
        """
        self.logger = logging.getLogger("context.graph_store")

        if db_path is None:
            default_dir = Path.home() / ".ai-orchestrator"
    ...
    def _init_schema(self) -> None:
        ...
        self.logger.info(f"Graph store initialized at {self.db_path}")
    ...
    def add_node(self, node: Node) -> str:
        ...
        with self._transaction() as cursor:
            cursor.execute(
                ...
                (id, node_type, title, content, metadata, tags, created_at, updated_at, importance_score, extra_data)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    node.id,
    ...
    def add_edge(self, edge: Edge) -> str:
        ...
        self.logger.debug(
            f"Added edge {edge.source_id} --[{edge.edge_type.value}]--> {edge.target_id}"
        )
        return edge.id
    ...
    def get_neighbors(
        self, node_id: str, edge_type: EdgeType | None = None, direction: str = "both"
    ...
    def query_nodes(
        ...
        query = f"""
            ...
            LIMIT ? OFFSET ?
        """  # noqa: B608  # parameterized with ? placeholders
    ...
    def traverse_graph(
        ...
        Returns:
            Dictionary with nodes and edges discovered
        """
