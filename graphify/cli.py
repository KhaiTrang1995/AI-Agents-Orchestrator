...
def scan(
    ...
    if report:
        ...
        console.print(f"[green]📄 Report:[/green] {os.path.join(output_dir, 'GRAPH_REPORT.md')}")
...
def stats(path: str, db: str | None) -> None:
    ...
    table.add_row("", "")
    for lang, count in sorted(data.get("languages", {}).items(), key=lambda x: x[1], reverse=True):
